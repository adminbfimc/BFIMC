...
const sessionSecret = process.env.SESSION_SECRET || 'change-this-bfimpc-local-session-secret';
...
const routes = {
  '/': { page: 'home', title: 'BFIMPC | Home' },
  '/portfolio': { page: 'portfolio', title: 'BFIMPC | Portfolio' },
  '/membership': { page: 'membership', title: 'BFIMPC | Membership' },
  '/services': { page: 'services', title: 'BFIMPC | Services' },
  '/contact': { page: 'contacts', title: 'BFIMPC | Contacts' },
  '/membership-form': { page: 'membership_form', title: 'BFIMPC | Membership Form' },
  '/loan-form': { page: 'loan_form', title: 'BFIMPC | Loan Form' }
};
...
if (require.main === module) {
  app.listen(port, () => {
    console.log(`BFIMPC Node site is running at http://localhost:${port}`);
  });
}
...
function renderAuth(res, mode = 'login', status, values = {}) {
  ...
  const messages = {
    registered: 'Your account is ready. Welcome to BFIMPC!',
  ...
  res.type('html').send(`${header('BFIMPC | Account', null)}${content}${footer()}`);
}
...
function renderForgotPassword(res, status) {
  const message = status === 'sent'
    ...
    : status === 'error'
      ...
      : '';
  res.type('html').send(`${header('BFIMPC | Reset Password', null)}${fragment('forgot_password').replace('{{AUTH_MESSAGE}}', message)}${footer()}`);
}
...
function renderResetPassword(res, token, status) {
  ...
  res.type('html').send(`${header('BFIMPC | Choose a New Password', null)}${content}${footer()}`);
}
...
function renderVerifyEmail(res, email, status) {
  ...
  res.type('html').send(`${header('BFIMPC | Verify Email', null)}${content}${footer()}`);
}

function renderAuthCallback(res) {
  const content = '<main class="account-page"><section class="account-shell single-account"><div class="account-card"><p class="section-kicker">Email verified</p><h1>Your account is confirmed.</h1><p class="account-lead">Your email has been successfully verified. You can now log in to your BFIMPC account.</p><a class="auth-submit" href="/auth?mode=login">Go to log in <i class="bi bi-arrow-right"></i></a></div></section></main>';
  res.type('html').send(`${header('BFIMPC | Account Confirmed', null)}${content}${footer()}`);
}
...
function renderProfile(res, user, status) {
  ...
  res.type('html').send(`${header('BFIMPC | Profile', user)}${content}${footer()}`);
}
...
async function renderAdmin(res, user, status, section = 'home', staffMode = false) {
  ...
  const pages = {
    ...
    users: `<section class="admin-heading"><p class="section-kicker">Account management</p><h1>User accounts</h1><p>View registered BFIMPC accounts and remove accounts when necessary.</p></section>${notice}<section class="admin-section admin-page-section"><div class="admin-section-heading"><h2>All registered users</h2><span>${counts.users} total</span></div><div class="admin-user-list">${userRows}</div></section>`
  };
  pages.home = `<section class="admin-heading"><h1>Dashboard overview</h1><p>A brief view of BFIMPC administration.</p></section><section class="admin-stats admin-summary" aria-label="Dashboard totals"><a href="/admin/content"><span class="stat-icon blue"><i class="bi bi-images"></i></span><div><strong>${counts.content}</strong><small>Total content posts</small><em>Manage content <i class="bi bi-arrow-right"></i></em></div></a><a href="/admin/affiliates"><span class="stat-icon orange"><i class="bi bi-people"></i></span><div><strong>${counts.affiliates}</strong><small>Affiliate partners</small><em>Manage partners <i class="bi bi-arrow-right"></i></em></div></a><a href="/admin/users"><span class="stat-icon green"><i class="bi bi-people"></i></span><div><strong>${counts.users}</strong><small>User accounts</small><em>Manage accounts <i class="bi bi-arrow-right"></i></em></div></a><a href="/admin/staff"><span class="stat-icon blue"><i class="bi bi-person-workspace"></i></span><div><strong>${counts.staff}</strong><small>Staff accounts</small><em>Manage staff <i class="bi bi-arrow-right"></i></em></div></a></section>`;
